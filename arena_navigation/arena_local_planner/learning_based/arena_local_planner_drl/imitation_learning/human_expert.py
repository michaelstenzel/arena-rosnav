...
""" human_expert.py
Record rollouts generated by a human expert. A rollout/episode/trajectory is a series of (observation, action) pairs recorded over the course of the
robot's journey from its starting position to the goal.
The agent will either be placed in randomly generated scenarios in a given map OR in a scenario specified by a json file.

In each step, the human expert chooses an action in the command line: action, a global variable, stores the last selected action. By pressing the 
a,s,d,w keys, the linear and angular velocities can be incremented or decremented. By pression o, the action will be set to standing still.
By pressing k, the action is confirmed, and a step is taken in the flatland environment using this action.
Detailed operating instructions are displayed at each step in the command line.

The synchronization of observation and action pairs occurs entirely in the while loop below: first a synchronized observation is sampled from the environment,
THEN the human expert chooses which action to take given the observation. This pair is then stored in the episode lists.

Each rollout will be saved to /output/human_expert/{map_name}_stage_{stage}/rollout_{datetime}.npz

Args:
    map_name (str): the name of the map used during recording. It will be used to create the path each episode is saved to.
    scenario (str): the name (including .json file extension) of a scenario file stored in /simulator_setup/scenarios. if this argument is set,
    the scenario task mode will be used with this scenario file. if it is not set, then the staged task mode will be used.
...
while(True):
    ...
    if done:
        ...
        if info['done_reason'] == 1:
            ...
            # done but not crashed - ran out of timesteps or reached the goal
            ...
            # or it will end the recording because all scenarios have been run their maximum number of times (the latter only applies to scenario task mode)
            try:
                env.reset()
                select_action()
